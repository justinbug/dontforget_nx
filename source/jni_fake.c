...
static FakeID *get_id(const char *cls, const char *name, const char *sig) {
  debugPrintf("JNI: get_id(class='%s', name='%s', sig='%s')\n", cls ? cls : "", name ? name : "", sig ? sig : "");
...
static juint call_int(const char *name, va_list va) {
  ...
  if (!strcmp(name, "getGuiWidth") || !strcmp(name, "getScreenWidth")) return screen_width;
  if (!strcmp(name, "getGuiHeight") || !strcmp(name, "getScreenHeight")) return screen_height;
  ...
  if (!strcmp(name, "GamepadsCount")) return 1;
  if (!strcmp(name, "GamepadGMLMapping")) return 0;
  if (!strcmp(name, "ShowQuestion")) return 0; // always "No"
  if (!strcmp(name, "HasVsyncHandler")) return 0; // no vsync handler
  ...
  if (!strcmp(name, "GetDefaultFrameBuffer")) return 0;
  (void)va;
  debugPrintf("JNI: call_int('%s') UNHANDLED\n", name);
  return 0;
}

static juint call_bool(const char *name, va_list va) {
  debugPrintf("JNI: call_bool('%s')\n", name);
  ...
  if (!strcmp(name, "GamepadConnected")) {
    int dev = va_arg(va, int);
    if (dev == 0) return 1;
    return 0;
  }
  ...
  (void)va;
  debugPrintf("JNI: call_bool('%s') UNHANDLED\n", name);
  return 0;
}

static float call_float(const char *name, va_list va) {
  debugPrintf("JNI: call_float('%s')\n", name);
  ...
  (void)va;
  debugPrintf("JNI: call_float('%s') UNHANDLED\n", name);
  return 0.0f;
}

static double call_double(const char *name, va_list va) {
  debugPrintf("JNI: call_double('%s')\n", name);
  ...
  (void)va;
  debugPrintf("JNI: call_double('%s') UNHANDLED\n", name);
  return 0.0;
}

static juint call_long(const char *name, va_list va) {
  debugPrintf("JNI: call_long('%s')\n", name);
  (void)va;
  debugPrintf("JNI: call_long('%s') UNHANDLED\n", name);
  return 0;
}
...
static void *call_object(const char *name, va_list va) {
  ...
  if (!strcmp(name, "CallExtensionFunction")) return NULL; // no extensions
  ...
  (void)va;
  debugPrintf("JNI: call_object('%s') UNHANDLED\n", name);
  return NULL;
}
...
static void call_void(const char *name, va_list va) {
  ...
  if (!strcmp(name, "EnumerateGamepadDevices")) {
    if (!g_gamepad_added && g_onGPDeviceAdded) {
      void *jname = jni_make_string("Nintendo Switch Controller");
      void *jdesc = jni_make_string("Nintendo Switch Controller");
      typedef void (*onGPDeviceAddedFn)(void *, void *, int, void *, void *, int, int, int, int, int);
      ((onGPDeviceAddedFn)g_onGPDeviceAdded)(fake_env, NULL, 0, jname, jdesc, 0x028e, 0x045e, 6, 1, 0xffffffff);
      jni_delete_ref(jname);
      jni_delete_ref(jdesc);
      if (g_onGamepadChange) {
        typedef void (*onGamepadChangeFn)(void *, void *);
        ((onGamepadChangeFn)g_onGamepadChange)(fake_env, NULL);
      }
      g_gamepad_added = 1;
    }
    return;
  ...
  (void)va;
  debugPrintf("JNI: call_void('%s') UNHANDLED\n", name);
}
...
static void *j_GetStaticObjectField(void *env, void *cls, FakeID *fid) {
  (void)env; (void)cls;
  debugPrintf("JNI: GetStaticObjectField('%s')\n", fid->name);
  ...
  debugPrintf("JNI: GetStaticObjectField('%s') UNHANDLED\n", fid->name);
  return NULL;
}

static juint j_GetStaticIntField(void *env, void *cls, FakeID *fid) {
  (void)env; (void)cls;
  debugPrintf("JNI: GetStaticIntField('%s')\n", fid->name);
  if (!strcmp(fid->name, "SDK_INT")) {
    return 28;
  }
  debugPrintf("JNI: GetStaticIntField('%s') UNHANDLED\n", fid->name);
  return 0;
}
