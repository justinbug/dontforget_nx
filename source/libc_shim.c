...
static const char *sanitize_path(const char *path) {
  ...
  // Strip duplicate sdmc:
  const char *last_sdmc = strstr(path, "sdmc:");
  if (last_sdmc) {
    const char *next = last_sdmc;
    while ((next = strstr(next + 5, "sdmc:")) != NULL) {
      last_sdmc = next;
    }
    path = last_sdmc;
  }

  // Strip duplicate /switch/yoyo_nx/
  const char *p1 = strstr(path, "/switch/yoyo_nx/");
  if (p1) {
    const char *p2 = strstr(p1 + 1, "/switch/yoyo_nx/");
    if (p2) {
      path = p2;
    }
  }
...
int rename_fake(const char *oldpath, const char *newpath) {
  return rename(sanitize_path(oldpath), sanitize_path(newpath));
}
...
const char *ctype_fake_ptr = &fake_ctype[128];
...
const char *getprogname_fake(void) { return "yoyo_nx"; }
...
int getrusage_fake(int who, void *usage) { (void)who; (void)usage; return 0; }
int statfs_fake(const char *path, void *buf) { (void)path; (void)buf; return 0; }
int fstatfs_fake(int fd, void *buf) { (void)fd; (void)buf; return 0; }
int fstatvfs_fake(int fd, void *buf) { (void)fd; (void)buf; return 0; }
int sigaction_fake(int sig, const void *act, void *oact) { (void)sig; (void)act; (void)oact; return 0; }
